...
# -*- coding: utf-8 -*-
"""
second_experiment.py — Resource-contention benchmark (production-ready)

What’s new vs your previous version:
- Keeps the original orchestrator/LLM flow and all aggregate CSV artifacts.
- Adds *per-cycle* timeline instrumentation so you can make timeline-style plots:
  writes results/timeline_contention.csv with schema:
    scenario,seed,p,cycle,tasks_completed,conflict_this_cycle,conflicts_cumulative
- Clean logging and robust defaults.

Run:
  python second_experiment.py --llm off --mode rules-only --tasks 15 --p-override 0.1 0.3 0.5 0.7 0.9
"""
...
# Optional BeeAI Framework Imports (kept optional)
try:
...
# Data handling and plotting deps
try:
    import pandas as pd
...
# -------------------------------
# Matplotlib defaults (publication-friendly)
# -------------------------------
mpl.rcParams.update({
    "figure.dpi": 300,
    "savefig.dpi": 300,
...
# FIX: Namespacing output files to prevent schema conflicts with other experiments.
THROUGHPUT_FILE = os.path.join(RESULTS_DIR, "throughput_contention.csv")
MAKESPAN_FILE   = os.path.join(RESULTS_DIR, "makespan_contention.csv")
CONFLICTS_FILE  = os.path.join(RESULTS_DIR, "conflicts_contention.csv")
ABLATIONS_FILE  = os.path.join(RESULTS_DIR, "ablations_contention.csv")
OVERHEAD_FILE   = os.path.join(RESULTS_DIR, "overhead_contention.csv")
TIMELINE_CONTENTION_FILE = os.path.join(RESULTS_DIR, "timeline_contention.csv")
...
class AutoSelfOrchestrator:
    """LLM-enabled orchestrator (optional)."""
    ...
    def _build_few_shot_prompt(self, task1: Task, task2: Task) -> str:
        return f"""
You are an expert AutoSelf Orchestrator Agent. Your task is to analyze two tasks and determine if they can be executed concurrently. A conflict exists if they both require the same critical resource 'R'.

Respond ONLY with a JSON object that strictly adheres to the following schema:
{{
  "conflict_detected": boolean,
  "reasoning": "A brief explanation of why a conflict was or was not detected."
}}

---
Here is an example of a valid response for a conflict:
{{
  "conflict_detected": true,
  "reasoning": "Both Task 1 and Task 2 require the critical resource 'R' and therefore cannot be executed concurrently."
}}
---

Now, analyze the following tasks and respond with your JSON object:
* Task 1: {task1.description} (needs R: {task1.needs_R})
* Task 2: {task2.description} (needs R: {task2.needs_R})
"""
    ...
    async def verify_tasks_for_conflict(self, task1: Task, task2: Task) -> Optional[bool]:
        ...
        async with self.semaphore:
            try:
                ...
                self._log(f"LLM verification failed: {repr(e)}", level="error")
                return None


# -------------------------------
# Simulation logic (adds per-cycle timeline instrumentation)
# -------------------------------
async def run_conflict_simulation(
    ...
    # ---- minimal instrumentation: start ----
    timeline: List[Dict[str, Any]] = []
    conflicts_cum = 0
    tasks_cum = 0
    # ---- minimal instrumentation: end ----
    ...
    while tasks_completed < len(tasks):
        ...
        # Remember queue length at start of cycle (for executed=2 vs 1)
        prev_q_len = len(task_queue)

        # Single remaining task case
        if prev_q_len == 1:
            tasks_completed += 1
            task_queue.pop(0)
            # ---- timeline log for single-task cycle ----
            conflict_this_cycle = False
            executed = 1
            tasks_cum += executed
            timeline.append({
                "scenario": "autoself" if orchestrator else "baseline",
                "seed": seed,
                "p": conflict_probability,
                "cycle": cycles,
                "tasks_completed": tasks_cum,
                "conflict_this_cycle": int(conflict_this_cycle),
                "conflicts_cumulative": conflicts_cum
            })
            continue
        ...
        if orchestrator and llm_gated:
            ...
            # ---- minimal instrumentation: per-cycle record ----
            conflict_this_cycle = bool(conflict_flag)
            executed = 2 if (not conflict_this_cycle and prev_q_len >= 2) else 1
            tasks_cum += executed
            if conflict_this_cycle:
                conflicts_cum += 1
            timeline.append({
                "scenario": "autoself" if orchestrator else "baseline",
                "seed": seed,
            ...
            # Baseline: run two at once; if both need R -> conflict (one finishes)
            t_rules = time.perf_counter()
            ...
            # ---- minimal instrumentation: per-cycle record ----
            conflict_this_cycle = bool(is_conflict)
            executed = 2 if (not conflict_this_cycle and prev_q_len >= 2) else 1
            tasks_cum += executed
            if conflict_this_cycle:
                conflicts_cum += 1
    ...
    log.info(
        f"Finished simulation for p={conflict_probability}, seed={seed}. "
        f"Cycles={cycles}, Conflicts={conflicts_encountered}, Throughput={throughput:.4f}"
    )

    # ---- minimal instrumentation: persist timeline to CSV ----
    os.makedirs(RESULTS_DIR, exist_ok=True)
    tl_path = TIMELINE_CONTENTION_FILE
    pd.DataFrame(timeline).to_csv(
        tl_path, index=False, mode="a",
        header=not os.path.exists(tl_path)
    )
    # ---- end instrumentation ----
...
class ExperimentRunner:
    ...
    def _generate_throughput_plot(self, df: pd.DataFrame) -> None:
        ...
        for s in strategies:
            ...
            if ds.empty:
                continue
            line = plt.plot(ds["p"], ds["mean"], marker="o", linestyle="-", label=s)[0]
            plt.fill_between(ds["p"], ds["mean"] - ds["ci95"], ds["mean"] + ds["ci95"], alpha=0.2, color=line.get_color())
            ...
            for p in xs:
                ...
                if len(y) > 0:
                    ...
                    plt.scatter(xj, y, s=18, alpha=0.6, color=line.get_color())
    ...
    def _generate_effect_size_plot(self, df: pd.DataFrame) -> None:
        ...
        piv = df.pivot_table(index=["conflict_probability", "total_cycles"], columns="strategy",
                             values="throughput", aggfunc="mean").reset_index()
    ...
    def _generate_overhead_bar(self) -> None:
        ...
        # FIX: Make reader more robust to handle different schemas from different experiments.
        df_oh = None
        # Prioritize the new, specific file, but fall back to the old one if needed.
        if os.path.exists(OVERHEAD_FILE):
            try:
                # Use engine='python' as it's more tolerant of ragged rows.
                df_oh = pd.read_csv(OVERHEAD_FILE, engine="python")
                log.info(f"Successfully loaded overhead data from '{OVERHEAD_FILE}'")
            except Exception as e:
                log.warning(f"Failed to read '{OVERHEAD_FILE}': {e}")
        
        if df_oh is None:
            log.warning(f"'{OVERHEAD_FILE}' not found or failed to load; skipping overhead plot.")
            return

        # Explicitly select only the columns we need to prevent errors from extra columns.
        required_cols = ["scenario", "rules_ms", "sim_ms", "llm_ms", "correction_ms"]
        available_cols = [col for col in required_cols if col in df_oh.columns]
        
        if "scenario" not in available_cols:
            log.warning("Overhead file is missing the 'scenario' column; skipping plot.")
            return
            
        df_oh = df_oh[available_cols].copy()
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="AutoSelf resource-contention benchmark (production)")
